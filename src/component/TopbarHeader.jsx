...
export const TopbarHeader = ({ toggleSidebarVisibility }) => {
  const [showProfile, setShowProfile] = useState(false);
  const [showLanguages, setShowLanguages] = useState(false);


  const toggleShowLanguages = () => { setShowLanguages(oldState => !oldState) }
  const toggleShowProfile = () => { setShowProfile(oldState => !oldState) }
    ...
    <header className="topbar-nav">
      <nav className="navbar navbar-expand fixed-top">
        ...
        <ul className="navbar-nav mr-auto align-items-center">
          ...
          <li className="nav-item" onClick={toggleSidebarVisibility}>
        ...
        <ul className="navbar-nav align-items-center right-nav-link">
          ...
          <li className={`nav-item language ${showLanguages && 'show'}`}>
            <a className="nav-link dropdown-toggle dropdown-toggle-nocaret waves-effect" data-toggle="dropdown"
              href="javascript:void();" onClick={toggleShowLanguages}><i className="fa fa-flag"></i></a>
            <ul className={`dropdown-menu dropdown-menu-right ${showLanguages && 'show'}`}>
          ...
          <li className={`nav-item ${showProfile && 'show'}`}>
            <a className="nav-link dropdown-toggle dropdown-toggle-nocaret" data-toggle="dropdown" href="#" onClick={toggleShowProfile}>
              <span className="user-profile"><img src="https://via.placeholder.com/110x110" className="img-circle" alt="user avatar" /></span> </a>

            <ul className={`dropdown-menu dropdown-menu-right ${showProfile && 'show'}`}>
